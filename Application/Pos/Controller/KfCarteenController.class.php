...
class KfCarteenController extends CoinController
{
    /*
   * var $userid 充值操作员id
   *
   ...
   */
    ...
    {
        parent::_initialize();
        $this->userid       = C('kfcarteen_userid');
        $this->cardPanterid = C('kfcarteen_cardpanterid');
        $this->chongPanterid=C('kfcarteen_chongpanterid');
        $this->parent       =C('kfcarteen_parent');
        $this->tradetype    ='30';
        $this->reFoods      ='31';
        //紧急预案商户号
        $this->emPanterid=C('kfcarteen_empanterid');
        $this->frozen=5;
        //前台紧急扣款
        $this->QemPanterid=C('kfcarteen_qempanterid');
        //数字签名
        $this->keycode = 'kfc0002kfc';
        $this->gift    = C('kfcarteen_gift');
    }
    //单卡充值
    public function cardRecharge(){
        ...
        if($customInfo['customlevel']=='开封食堂团购客户'){
        ...
        //新增 对不退卡的充值管理
        if($customInfo['num']==='0'&& $customInfo['cardflag']==='on'){
            $num = $customInfo['num'];
        }else{
   ...
   */
    protected function exChong( $map){
        ...
        $orderSql="INSERT INTO kf_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
        $orderSql.="{$cardno},'{$num}','01','{$panterid}')";
    ...
    public function getConsume(){
        ...
        if($customInfo['customlevel']=='开封食堂团购客户'){
             $this->teamInfo($cardno, $customInfo);
//            $isteamFLag ='2';
...
//            $list=M('kf_team')->where(['teamid'=>$teamid])->select();
//            // var $list 所有的充值单号
...
//            returnMsg(['status'=>'1','codemsg'=>'查询成功','data'=>$res,'flag'=>$isteamFLag]);
    ...
    public function returnCard(){
        ...
        $orderSql="INSERT INTO kf_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
    ...
    public function goods(){
        ...
        $goods=M('kf_type')->alias('t')
            ->join('left join kf_goods g on g.type=t.type')
    ...
    public function getOrder(){
        ...
        foreach($order as $val){
            $list=M('kf_goods')->where(['goodsid'=>$val['goodsid'],'type'=>$val['type'],'panterid'=>$panterid])->find();
            if($val['type']=='1'){
    ...
    */
    ...
    public function detail(){
        ...
        $sql="(SELECT sum(num) as num,goodsname,price,goodsid,type,panterid,flag,cardno,cardnum from kf_order where cardno='{$map['cardno']}' AND cardnum='{$map['cardnum']}' ";
        ...
        $reSql="(SELECT sum(num) as num,goodsname,price,goodsid,type,panterid,flag,cardnum,cardno from kf_order where cardno='{$rewhere['cardno']}' AND cardnum='{$rewhere['cardnum']}' ";
    ...
    protected function searchChong($method,$map){
        $map['flag']='01';
        $orders=M('kf_order')->where($map)->select();
        if($orders==false){
    ...
    private function getRefund($map){
        $map['flag']='04';
        $orders=M('kf_order')->where($map)->field('tradeid')->select();
        if($orders==false){
            return 0;
        }
        $tradeWhere=array_column($orders,'tradeid');
    ...
    private function combineDetail($map){
        ...
        $res=$this->model->table('kf_combine')->where($whereIn)->field('sum(amount) as amount')->select();
        $in = $res[0]['amount']?:0;
        $whereOut=['outcardno'=>$map['cardno'],'outnum'=>$map['num'],'status'=>'1'];
        $resOut=$this->model->table('kf_combine')->where($whereOut)->field('sum(amount) as amount')->select();
        $out = $resOut[0]['amount']?:0;
        return ['in'=>$in,'out'=>$out];
    }
    // 查询单卡的交易详情
    private function orderInfo($map){
        $orderlist=M('kf_order')->where($map)
...
//---------------------------------------------分割线---------------------------------------------------
    ...
    public function chongDatail(){
        ...
        returnMsg(['status'=>'1','codemsg'=>'查询成功','data'=>['noraml'=>$noraml,'gift'=>$gift,'refund'=>$refund]]);
    }
    //结算充值 合计总金额
    private function chargeSum($map){
        $chargeInfo = $this->model->table('custom_purchase_logs')->alias('cup')
                                  ->join('left join kf_order kf on kf.tradeid=cup.purchaseid')
 ...
 //------------------------------------------------------分割线------------------------------------
    public function emergency(){
        ...
        $bool=M('kf_goods')->where($map)->find();
        if($bool==false){
            $goodsSql="INSERT INTO kf_goods(goodsid,panterid,type,goodsname,status) VALUES ('1','{$panterid}','2','特殊消费','1')";
            $typeSql="INSERT INTO kf_type(panterid,type,name,status) VALUES ('{$panterid}','2','特殊','1')";
        ...
        $ordeSql="INSERT INTO kf_order(tradeid,cardno,cardnum,goodsid,type,price,num,flag,placeddate,placedtime,panterid,goodsname) VALUES ";
   ...
   */
    ...
    private function exTeamCharge($map,$info){
        ...
        foreach($info as $key =>$val){
            ...
            //修改卡使用次数
            $customNumIf=M('customs')->where(['customid'=>$val['customid']])->save(['num'=>$val['num'],'cardflag'=>'on','customlevel'=>'开封食堂团购客户']);
            if(!$customNumIf){
                $this->model->rollback();
            ...
            if($key>=1){
                ...
                $orderSql.=" INTO kf_order (tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
                $orderSql.="{$val['cardno']},'{$val['num']}','01','{$map['panterid']}')";

                $teamSql.=" INTO kf_team(teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                ...
                //充值单记录
                $orderSql.="INSERT ALL INTO kf_order (tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
                $orderSql.="{$val['cardno']},'{$val['num']}','01','{$map['panterid']}')";
                // 团冲记录写入
                $teamSql.="INSERT ALL INTO kf_team (teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                $teamSql.="'{$map['teamid']}','{$purchaseid}','{$placddate}','{$placdtime}','1')";
            }
            // 修改账户金额
        ...
        //团冲返点记录
        $rateSql = "insert into kf_team_rate VALUES ('{$map['teamid']}','{$placddate}','{$placdtime}','{$map['rate']}')";

        $rateif  =  $this->model->execute($rateSql);
        $customplIf=$this->model->execute($customplSql);
        $auditlogsIf=$this->model->execute($auditlogsSql);
        $cardplIf=$this->model->execute($cardplSql);
        $orderif=$this->model->execute($orderSql);
        ...
        if($customplIf==true && $auditlogsSql==true && $cardplIf==true && $orderif==true && $teamif==true && $accountif==true && $rateif==true){
            return true;
        }else{
            $this->model->rollback();
    ...
    private function getTeamid($map){
        $map['flag']='01';
        $res=$this->model->table('kf_order')->where($map)->field('tradeid')->find();
        if($res==true){
            $teamlist=$this->model->table('kf_team')->where(['purchaseid'=>trim($res['tradeid']),'flag'=>'1'])->field('teamid')->find();
            if($teamlist==true){
    ...
    private function teamConsume($customlist){
        ...
        foreach($customlist as $val){
            ...
            $xiao=$this->model->table('kf_order')->where($xiaoWhere)->field('price,num')->select();
            if($xiao){
                foreach($xiao as $v){
            ...
            $tuicai=$this->model->table('kf_order')->where($tuiWhere)->field('price,num')->select();
            if($tuicai==true){
    ...
    */
    private function teamCombine($customlist){
        ...
        foreach($customlist as $val){
            ...
            $inlist=$this->model->table('kf_combine')->where($inWhere)->field('amount')->select();
            $in=0;
            if($inlist==true){
            ...
            $outlist=$this->model->table('kf_combine')->where($outWhere)->field('amount')->select();
            $out=0;
            if($outlist==true){
...
//        $panterid='00000241';
//        $cardno ='6880374800000000021';
//        $termno ='24100007';
//        $getTeamid=md5('team_id201704111701270020');
        $panterid==$this->chongPanterid||returnMsg(array('status'=>'02','codemsg'=>'此商户没有退卡权限'));
        $cardInfo   = $this->cardInfo(__FUNCTION__,$cardno);
        $customInfo = $this->customInfo(__FUNCTION__,$cardInfo['customid']);
        $customInfo['customlevel']=='开封食堂团购客户'||returnMsg(array('status'=>'07','codemsg'=>'此卡号不是团购卡:'.$cardno.'不能退卡'));
        $mapTeam = ['cardno'=>$cardno,'cardnum'=>$customInfo['num']];
        $teamid = $this->getTeamid($mapTeam);
    ...
    private function cardTeamInfo($method,$teamid){
        $lists = M('kf_team')->where(['teamid'=>$teamid])->select();
        $count = count($lists);
    ...
    */
    private function exTeamCards($info,$map,$cardno){
        ...
        foreach($info as $key => $val){
            ...
            if($key>=1){
                ...
                $orderSql.=" INTO kf_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
                ...
                $sql.="'{$termno}','{$termno}','{$panterid}','{$tradeid}','{$placeddate}','{$val['amount']}','0','{$val['customid']}','{$val['cardno']}','{$placedtime}','{$tradetype}','{$tac}','0')";
                // kf_order 表
                $orderSql.="INSERT ALL INTO kf_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
                $orderSql.="'{$val['cardno']}','{$val['num']}','03','{$panterid}')";
                //kf_team 表
                $teamSql.="INSERT ALL INTO kf_team (teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                $teamSql.="'{$teamid}','{$tradeid}','{$placeddate}','{$placedtime}','2')";
    ...
    //团充值 时验证返点
    private function getTeamRate($value){
        if($value==='0'){
            return 0;
        }elseif($value==='1'){
            return 10;
        }else{
            return(['status'=>'09','codemsg'=>'非法返点信息传入']);
        }
    }
...
//------------------------------------------------------菜品处理----------------------------
    ...
    private function addGoods($map){
        $goodslist=M('kf_goods')->where(['panterid'=>$map['panterid']])->select();
        if($goodslist==false){
            $goodsid=1;
        }else{
            $namelist=M('kf_goods')->where(['goodsname'=>$map['goodsname'],'panterid'=>$map['panterid'],'type'=>$map['type']])->find();
            if($namelist==true){
        ...
        $sql="INSERT INTO kf_goods (goodsid,panterid,type,goodsname,price,status) VALUES ('{$goodsid}','{$map['panterid']}','{$map['type']}','{$map['goodsname']}','{$map['price']}','1')";
 ...
 //------------------------------大食堂其他方法
    ...
    public function weekData(){
        ...
        if($panterid==C('kfcarteen_getDrink')){
            ...
            $list=M('kf_order')->where($map)->field('nvl(sum(price*num),0) esum,placeddate,flag')
                ->group('placeddate,flag')->order('placeddate desc')->select();
            //退菜的
            $map['flag']='04';
            $refundlist=M('kf_order')->where($map)->field('nvl(sum(price*num),0) esum,placeddate,flag')
                ->group('placeddate,flag')->order('placeddate desc')->select();
            //第一步 有退菜日期的 获取  与正常合并
    ...
    private function getDrink($panterid){
        ...
        $map['panterid']=$panterid;
        $map['flag']='02';
        $getlist=M('kf_order')->where($map)->field('sum(price*num) as csum,sum(num) as tnum,goodsname,goodsid,type,placeddate')
            ->group('goodsname,goodsid,type,placeddate')->select();
        //退菜
        $map['flag']='04';
        $refulist=M('kf_order')->where($map)->field('sum(price*num) as csum,sum(num) as tnum,goodsname,goodsid,type,placeddate')
            ->group('goodsname,goodsid,type,placeddate')->select();
        if($refulist==true){
...
//---------------------------------------------公用方法
    ...
    private function cardInfo($method,$cardno){
        if($cardInfo = $this->model->table('cards')->where(['cardno'=>$cardno])->find()){
            ...
            if(substr($cardno,3,4)!='0378'){
                $error= ['status'=>'02','codemsg'=>'此卡号:'.$cardno.'不属于开封地区'];
                $this->errorHandle($method,$error,'查询卡号信息操作');
            }
            return $cardInfo;
        }else{
    ...
    private function writeLog($dir,$data){
        ...
        $str.="\n";
        $filename=PUBLIC_PATH.'logs/kfcarteen/'.$dir.'/'.date('Ymd',time()).'.txt';
        $this->getDir($filename);
        file_put_contents($filename,$str,FILE_APPEND);
    }
    //合计金额
    ...
    public function giftRate()
    {
        ...
        returnMsg(['status'=>'1','gift'=>C('kfcarteen_gift_api')]);
    }
    //特惠充值
    public function rechargeGift(){
        ...
        if($bool['msg']===true && $customIf==true){
            $this->model->commit();
            $balanceInfo = $this->model->table('account')->where(['type'=>'00','customid'=>$cardInfo['customid']])->field('amount')->find();
            $this->writeLog('single+success',['cardno'=>$cardno,'cardnum'=>'0','purchaseId'=>$bool['purchaseid'],'balance'=>$balanceInfo['amount'],'org'=>$cardInfo['amount']]);
    ...
    protected function exGiftChong( $map){
        ...
        $orderSql="INSERT INTO kf_order(tradeid,cardno,cardnum,flag,panterid,price) VALUES ('{$purchaseid}',";
   ...
   //---------新团冲执行------------------------------------------------------------------------------
    public function teamCharge(){
        ...
        $this->model->startTrans();
        $bool=$this->newexTeamCharge($map,$cardinfo);
        if($bool==true){
            $this->model->commit();
            $this->writeLog('teamcharge_susccess',['teamid'=>$teamid,'cardlist'=>json_encode($cardlist)]);
            returnMsg(['status'=>'1','codemsg'=>'团购成功','teamid'=>$teamid]);
    ...
    private function newexTeamCharge($map,$info){
        $currentDate  = date('Ymd');
        $checkDate    = date('Ymd');
        $placddate    = date('Ymd');
        $placdtime    = date('H:i:s');
        ...
        $sql        = "UPDATE customs set num=nvl(num,0)+1,cardflag='on',customlevel='开封食堂团购客户' where customid in ($inCustomid)";
        ...
        $accountif  = $this->model->table('account')
                           ...
                           ->save(['amount'=>$map['amount']]);
        if($customNumIf==true && $customplIf==true && $auditlogsIf==true && $cardplIf==true && $orderif==true && $teamif==true && $accountif==true && $rateif==true){
            return true;
        }else{
            $this->model->rollback();
            returnMsg(['status'=>'07','codemsg'=>'数据库写入失败不能充卡']);
    ...
    //退卡前 返回团卡的信息
    private function teamInfo($cardno, $customInfo){
        $isteamFLag = '2';
        $mapTeam    = ['cardno'=>$cardno,'cardnum'=>$customInfo['num']];
        //获取团冲单号
        $teamid     = $this->getTeamid($mapTeam);
        ...
        $where     = ['purchaseid'=>['in',$purlist],'userid'=>$this->userid];
        //充值总金额 $chongSum
        $chong     = M('card_purchase_logs')->where($where)->field('nvl(sum(amount),0) amount')->select();
        $chongSum  = $chong[0]['amount'];

        //查询消费总金额 $conSum
        $subSql   = M('kf_team')->alias('team')
                                ->join('left join kf_order k_o on team.purchaseid=k_o.tradeid')
                                ->field('k_o.cardno,k_o.cardnum')
                                ->where(['team.teamid'=>$teamid])->buildSql();
        //消费合计
        $consume = $this->model->table($subSql." sub")
                                ->join("left join kf_order kf on sub.cardno=kf.cardno and sub.cardnum=kf.cardnum")
    ...
    private function newteamCombine($subSql){
        $in  = $this->model->table($subSql." sub")
                            ->join('left join kf_combine kf on sub.cardno=kf.cardno and sub.cardnum=kf.innum')
                            ->field('nvl(sum(amount),0) incard')
                            ->select();
        $out = $this->model->table($subSql." sub")
                            ->join('left join kf_combine kf on sub.cardno=kf.outcardno and sub.cardnum=kf.outnum')
    ...
    private function newcardTeamInfo($method,$teamid){
        $count = M('kf_team')->where(['teamid'=>$teamid])->count();
        $info = $this->model->table('kf_team')->alias('kf')
                            ->join('kf_order kd on kf.purchaseid=kd.tradeid')
