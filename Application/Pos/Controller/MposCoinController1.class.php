  ...
  public function consume(){
      ...
      if($balanceConsumeIf==true&&$coinConsumeIf==true){
          ...
          if(floatval($coinAmount)>0){
            ...
            $panter=M('panters')->where("panterid={$panterid}")->find();
            $tpl_value="#content#=".urlencode("会员通宝消费成功！会员名字是:{$customInfo['namechinese']};手机号:{$customInfo['linktel']};在{$panter['namechinese']}消费");
            $nowmobile = "18538295100,18697323783";
            $re = $this->tpl_send("1294397",$nowmobile,$tpl_value);
          ...
          echo json_encode(array('status'=>'1','info'=>array('reduceBalance'=>floatval($balanceAmount),'reduceCoin'=>floatval($coinAmount)),
                                  'cointradidlist'=>$coinConsumeIf,'cashtradidlist'=>$balanceConsumeIf,'time'=>time()));
      }else{
          $this->model->rollback();
