...
class HebiController extends CoinController
{
    /*
   * var $userid 充值操作员id
   *
   ...
   */
    ...
    {
        parent::_initialize();
        $this->userid       = C('hb_userid');
        $this->cardPanterid = C('hb_cardpanterid');
        $this->chongPanterid=C('hb_chongpanterid');
        $this->parent       =C('hb_parent');
        $this->tradetype    ='30';
        $this->reFoods      ='31';
        //紧急预案商户号
        $this->emPanterid=C('hb_empanterid');
        $this->frozen=5;
        //前台紧急扣款
        $this->QemPanterid=C('hb_qempanterid');
        //数字签名
    ...
    public function cardRecharge(){
        ...
        if($customInfo['customlevel']=='鹤壁食堂团购客户'){
   ...
   */
    protected function exChong( $map){
        ...
        $orderSql="INSERT INTO hb_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
    ...
    public function getConsume(){
        ...
        if($customInfo['customlevel']=='鹤壁食堂团购客户'){
            ...
            $list=M('hb_team')->where(['teamid'=>$teamid])->select();
            // var $list 所有的充值单号
    ...
    public function returnCard(){
        ...
        $orderSql="INSERT INTO hb_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
    ...
    public function goods(){
        ...
        $goods=M('hb_type')->alias('t')
            ->join('left join hb_goods g on g.type=t.type')
    ...
    public function getOrder(){
        ...
        foreach($order as $val){
            $list=M('hb_goods')->where(['goodsid'=>$val['goodsid'],'type'=>$val['type'],'panterid'=>$panterid])->find();
            if($val['type']=='1'){
    ...
    protected function searchGoodsClass($map){
        $sql="SELECT sum(num) as num,goodsname,price,goodsid,type from hb_order where type='{$map['type']}' AND panterid='{$map['panterid']}' AND cardno='{$map['cardno']}' AND cardnum='{$map['cardnum']}' ";
    ...
    */
    private function refundNum($val,$panterid,$cardno,$cardnum){
        ...
        $mapNum['flag']='02';
        $consumeSum=M('hb_order')->where($mapNum)->select();
        if($consumeSum==true){
        ...
        //查询退菜数量
        $mapNum['flag']='04';
        $refundSum=M('hb_order')->where($mapNum)->select();
        if($refundSum==false){
            return $getNum;
        }else{
    ...
    public function detail(){
        ...
        $sql="(SELECT sum(num) as num,goodsname,price,goodsid,type,panterid,flag,cardno,cardnum from hb_order where cardno='{$map['cardno']}' AND cardnum='{$map['cardnum']}' ";
        ...
        $reSql="(SELECT sum(num) as num,goodsname,price,goodsid,type,panterid,flag,cardnum,cardno from hb_order where cardno='{$rewhere['cardno']}' AND cardnum='{$rewhere['cardnum']}' ";
    ...
    protected function searchChong($method,$map){
        $map['flag']='01';
        $orders=M('hb_order')->where($map)->select();
        if($orders==false){
    ...
    private function getRefund($map){
        $map['flag']='04';
        $orders=M('hb_order')->where($map)->field('tradeid')->select();
        if($orders==false){
            return 0;
        }
        $tradeWhere=array_column($orders,'tradeid');
    ...
    private function combineDetail($map){
        ...
        $res=$this->model->table('hb_combine')->where($whereIn)->field('sum(amount) as amount')->select();
        $in = $res[0]['amount']?:0;
        $whereOut=['outcardno'=>$map['cardno'],'outnum'=>$map['num'],'status'=>'1'];
        $resOut=$this->model->table('hb_combine')->where($whereOut)->field('sum(amount) as amount')->select();
        $out = $resOut[0]['amount']?:0;
        return ['in'=>$in,'out'=>$out];
    }
    // 查询单卡的交易详情
    private function orderInfo($map){
        $orderlist=M('hb_order')->where($map)
...
//---------------------------------------------分割线---------------------------------------------------
    ...
    private function chargeSum($map){
        $chargeInfo = $this->model->table('custom_purchase_logs')->alias('cup')
                                  ->join('left join hb_order kf on kf.tradeid=cup.purchaseid')
 ...
 //------------------------------------------------------分割线------------------------------------
    public function emergency(){
        ...
        $bool=M('hb_goods')->where($map)->find();
        if($bool==false){
            $goodsSql="INSERT INTO hb_goods(goodsid,panterid,type,goodsname,status) VALUES ('1','{$panterid}','2','特殊消费','1')";
            $typeSql="INSERT INTO hb_type(panterid,type,name,status) VALUES ('{$panterid}','2','特殊','1')";
        ...
        $ordeSql="INSERT INTO hb_order(tradeid,cardno,cardnum,goodsid,type,price,num,flag,placeddate,placedtime,panterid,goodsname) VALUES ";
    ...
    private function getCombine($map,$cardlist){
        foreach($cardlist as $key=>$val){
            ...
            $sql="INSERT INTO hb_combine (cardno,innum,outcardno,outnum,placeddate,placedtime,status,tradeid,amount) VALUES (";
   ...
   */
    ...
    private function exTeamCharge($map,$info){
        ...
        foreach($info as $key =>$val){
            ...
            //修改卡使用次数
            $customNumIf=M('customs')->where(['customid'=>$val['customid']])->save(['num'=>$val['num'],'cardflag'=>'on','customlevel'=>'鹤壁食堂团购客户']);
            if(!$customNumIf){
                $this->model->rollback();
            ...
            if($key>=1){
                ...
                $orderSql.=" INTO hb_order (tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
                $orderSql.="{$val['cardno']},'{$val['num']}','01','{$map['panterid']}')";

                $teamSql.=" INTO hb_team(teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                ...
                //充值单记录
                $orderSql.="INSERT ALL INTO hb_order (tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
                $orderSql.="{$val['cardno']},'{$val['num']}','01','{$map['panterid']}')";
                // 团冲记录写入
                $teamSql.="INSERT ALL INTO hb_team (teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                $teamSql.="'{$map['teamid']}','{$purchaseid}','{$placddate}','{$placdtime}','1')";
            }
            // 修改账户金额
    ...
    private function getTeamid($map){
        $map['flag']='01';
        $res=$this->model->table('hb_order')->where($map)->field('tradeid')->find();
        if($res==true){
            $teamlist=$this->model->table('hb_team')->where(['purchaseid'=>trim($res['tradeid']),'flag'=>'1'])->field('teamid')->find();
            if($teamlist==true){
    ...
    private function teamConsume($customlist){
        ...
        foreach($customlist as $val){
            ...
            $xiao=$this->model->table('hb_order')->where($xiaoWhere)->field('price,num')->select();
            if($xiao){
                foreach($xiao as $v){
            ...
            $tuicai=$this->model->table('hb_order')->where($tuiWhere)->field('price,num')->select();
            if($tuicai==true){
    ...
    */
    private function teamCombine($customlist){
        ...
        foreach($customlist as $val){
            ...
            $inlist=$this->model->table('hb_combine')->where($inWhere)->field('amount')->select();
            $in=0;
            if($inlist==true){
            ...
            $outlist=$this->model->table('hb_combine')->where($outWhere)->field('amount')->select();
            $out=0;
            if($outlist==true){
    ...
    public function returnTeam(){
        ...
        $cardInfo   = $this->cardInfo(__FUNCTION__,$cardno);
        $customInfo = $this->customInfo(__FUNCTION__,$cardInfo['customid']);
        $customInfo['customlevel']=='鹤壁食堂团购客户'||returnMsg(array('status'=>'07','codemsg'=>'此卡号不是团购卡:'.$cardno.'不能退卡'));
        $mapTeam = ['cardno'=>$cardno,'cardnum'=>$customInfo['num']];
        $teamid = $this->getTeamid($mapTeam);
    ...
    private function cardTeamInfo($method,$teamid){
        $lists = M('hb_team')->where(['teamid'=>$teamid])->select();
        $count = count($lists);
    ...
    private function exTeamCards($info,$map,$cardno){
        ...
        foreach($info as $key => $val){
            ...
            if($key>=1){
                ...
                $orderSql.=" INTO hb_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
                ...
                $sql.="'{$termno}','{$termno}','{$panterid}','{$tradeid}','{$placeddate}','{$val['amount']}','0','{$val['customid']}','{$val['cardno']}','{$placedtime}','{$tradetype}','{$tac}','0')";
                // hb_order 表
                $orderSql.="INSERT ALL INTO hb_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
                $orderSql.="'{$val['cardno']}','{$val['num']}','03','{$panterid}')";
                //hb_team 表
                $teamSql.="INSERT ALL INTO hb_team (teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                $teamSql.="'{$teamid}','{$tradeid}','{$placeddate}','{$placedtime}','2')";
...
//------------------------------------------------------菜品处理----------------------------
    ...
    private function addGoods($map){
        ...
        if($goodslist==false){
            ...
            $namelist=M('hb_goods')->where(['goodsname'=>$map['goodsname'],'panterid'=>$map['panterid'],'type'=>$map['type']])->find();
            if($namelist==true){
        ...
        $sql="INSERT INTO hb_goods (goodsid,panterid,type,goodsname,price,status) VALUES ('{$goodsid}','{$map['panterid']}','{$map['type']}','{$map['goodsname']}','{$map['price']}','1')";
    ...
    private function goodsInfo($map){
        if($gooodsInfo = M('hb_goods')->where($map)->find()){
            return $gooodsInfo;
        }else{
    ...
    private function updateGoodsStatus($where,$map){
        return M('hb_goods')->where($where)->save($map);
    }
 ...
 //------------------------------大食堂其他方法
    ...
    public function weekData(){
        ...
        if($panterid==C('hb_getDrink')){
            ...
            $list=M('hb_order')->where($map)->field('nvl(sum(price*num),0) esum,placeddate,flag')
                ->group('placeddate,flag')->order('placeddate desc')->select();
            //退菜的
            $map['flag']='04';
            $refundlist=M('hb_order')->where($map)->field('nvl(sum(price*num),0) esum,placeddate,flag')
                ->group('placeddate,flag')->order('placeddate desc')->select();
            //第一步 有退菜日期的 获取  与正常合并
    ...
    private function getDrink($panterid){
        ...
        $map['panterid']=$panterid;
        $map['flag']='02';
        $getlist=M('hb_order')->where($map)->field('sum(price*num) as csum,sum(num) as tnum,goodsname,goodsid,type,placeddate')
            ->group('goodsname,goodsid,type,placeddate')->select();
        //退菜
        $map['flag']='04';
        $refulist=M('hb_order')->where($map)->field('sum(price*num) as csum,sum(num) as tnum,goodsname,goodsid,type,placeddate')
            ->group('goodsname,goodsid,type,placeddate')->select();
        if($refulist==true){
...
//---------------------------------------------公用方法
    ...
    private function cardInfo($method,$cardno){
        if($cardInfo = $this->model->table('cards')->where(['cardno'=>$cardno])->find()){
            ...
            if(substr($cardno,3,4)!='0392'){
                $error= ['status'=>'02','codemsg'=>'此卡号:'.$cardno.'不属于鹤壁地区'];
                $this->errorHandle($method,$error,'查询卡号信息操作');
            }
            return $cardInfo;
        }else{
    ...
    private function valiPanterid($method,$panterid,$description){
        ...
        if($list['parent']!=$this->parent){
            $error=['status'=>'13','codemsg'=>'不是鹤壁大食堂商户'];
            $this->errorHandle($method,$error,$description);
        }
        return $list;
    }
    ...
    private function errorLog($method,$error,$description){
        $filename = PUBLIC_PATH.'logs/hb_catering_error/'.date('Ymd',time()).'.txt';
    ...
    private function writeLog($dir,$data){
        ...
        $str.="\n";
        $filename=PUBLIC_PATH.'logs/hb_catering/'.$dir.'/'.date('Ymd',time()).'.txt';
        $this->getDir($filename);
        file_put_contents($filename,$str,FILE_APPEND);
    }
    //合计金额
    ...
    public function giftRate()
    {
        ...
        returnMsg(['status'=>'1','gift'=>C('hb_gift_api')]);
    }
    ...
    protected function exGiftChong( $map){
        ...
        $orderSql="INSERT INTO hb_order(tradeid,cardno,cardnum,flag,panterid,price) VALUES ('{$purchaseid}',";
    ...
    */
    ...
    protected function cannotCombine($info){
        if($info['num']==='0'){
            returnMsg(['status'=>'09','codemsg'=>'特惠卡不能参与合卡消费']);
        }
    }
    //------------------------------------------------------------------特惠卡无卡底费 书写
