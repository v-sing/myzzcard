...
class ShenhouController extends CoinController
{
    /*
   * var $userid 充值操作员id
   *
   ...
   */
    ...
    {
        parent::_initialize();
        $this->userid       = C('shenhou_userid');
        $this->cardPanterid = C('shenhou_cardpanterid');
        $this->chongPanterid=C('shenhou_chongpanterid');
        $this->parent       =C('shenhou_parent');
        $this->tradetype    ='30';
        $this->reFoods      ='31';
        //紧急预案商户号
        $this->emPanterid=C('shenhou_empanterid');
        $this->frozen=5;
        //前台紧急扣款
        $this->QemPanterid=C('shenhou_qempanterid');
        //数字签名
    ...
    public function cardRecharge(){
        ...
        if($customInfo['customlevel']=='神垕食堂团购客户'){
   ...
   */
    protected function exChong( $map){
        ...
        $orderSql="INSERT INTO sh_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
    ...
    public function getConsume(){
        ...
        if($customInfo['customlevel']=='神垕食堂团购客户'){
            ...
            $list=M('sh_team')->where(['teamid'=>$teamid])->select();
            // var $list 所有的充值单号
    ...
    public function returnCard(){
        ...
        $orderSql="INSERT INTO sh_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
    ...
    public function goods(){
        ...
        $goods=M('sh_type')->alias('t')
            ->join('left join sh_goods g on g.type=t.type')
    ...
    public function getOrder(){
        ...
        foreach($order as $val){
            $list=M('sh_goods')->where(['goodsid'=>$val['goodsid'],'type'=>$val['type'],'panterid'=>$panterid])->find();
            if($val['type']=='1'){
                $orderSql="INSERT INTO sh_order(tradeid,cardno,cardnum,goodsid,type,goodsname,price,num,flag,placeddate,placedtime,panterid) VALUES ('{$tradeid}',";
    ...
    protected function searchGoodsClass($map){
        $sql="SELECT sum(num) as num,goodsname,price,goodsid,type from sh_order where type='{$map['type']}' AND panterid='{$map['panterid']}' AND cardno='{$map['cardno']}' AND cardnum='{$map['cardnum']}' ";
    ...
    */
    private function refundNum($val,$panterid,$cardno,$cardnum){
        ...
        $mapNum['flag']='02';
        $consumeSum=M('sh_order')->where($mapNum)->select();
        if($consumeSum==true){
        ...
        //查询退菜数量
        $mapNum['flag']='04';
        $refundSum=M('sh_order')->where($mapNum)->select();
        if($refundSum==false){
            return $getNum;
        }else{
    ...
    public function detail(){
        ...
        $sql="(SELECT sum(num) as num,goodsname,price,goodsid,type,panterid,flag,cardno,cardnum from sh_order where cardno='{$map['cardno']}' AND cardnum='{$map['cardnum']}' ";
        ...
        $reSql="(SELECT sum(num) as num,goodsname,price,goodsid,type,panterid,flag,cardnum,cardno from sh_order where cardno='{$rewhere['cardno']}' AND cardnum='{$rewhere['cardnum']}' ";
    ...
    protected function searchChong($method,$map){
        $map['flag']='01';
        $orders=M('sh_order')->where($map)->select();
        if($orders==false){
    ...
    private function getRefund($map){
        $map['flag']='04';
        $orders=M('sh_order')->where($map)->field('tradeid')->select();
        if($orders==false){
            return 0;
        }
        $tradeWhere=array_column($orders,'tradeid');
    ...
    private function combineDetail($map){
        ...
        $res=$this->model->table('sh_combine')->where($whereIn)->field('sum(amount) as amount')->select();
        $in = $res[0]['amount']?:0;
        $whereOut=['outcardno'=>$map['cardno'],'outnum'=>$map['num'],'status'=>'1'];
        $resOut=$this->model->table('sh_combine')->where($whereOut)->field('sum(amount) as amount')->select();
        $out = $resOut[0]['amount']?:0;
        return ['in'=>$in,'out'=>$out];
    }
    // 查询单卡的交易详情
    private function orderInfo($map){
        $orderlist=M('sh_order')->where($map)
...
//---------------------------------------------分割线---------------------------------------------------
    ...
    private function chargeSum($map){
        $chargeInfo = $this->model->table('custom_purchase_logs')->alias('cup')
                                  ->join('left join sh_order kf on kf.tradeid=cup.purchaseid')
 ...
 //------------------------------------------------------分割线------------------------------------
    public function emergency(){
        ...
        $bool=M('sh_goods')->where($map)->find();
        if($bool==false){
            $goodsSql="INSERT INTO sh_goods(goodsid,panterid,type,goodsname,status) VALUES ('1','{$panterid}','2','特殊消费','1')";
            $typeSql="INSERT INTO sh_type(panterid,type,name,status) VALUES ('{$panterid}','2','特殊','1')";
        ...
        $ordeSql="INSERT INTO sh_order(tradeid,cardno,cardnum,goodsid,type,price,num,flag,placeddate,placedtime,panterid,goodsname) VALUES ";
    ...
    private function getCombine($map,$cardlist){
        foreach($cardlist as $key=>$val){
            ...
            $sql="INSERT INTO sh_combine (cardno,innum,outcardno,outnum,placeddate,placedtime,status,tradeid,amount) VALUES (";
   ...
   */
    ...
    private function exTeamCharge($map,$info){
        ...
        foreach($info as $key =>$val){
            ...
            //修改卡使用次数
            $customNumIf=M('customs')->where(['customid'=>$val['customid']])->save(['num'=>$val['num'],'cardflag'=>'on','customlevel'=>'神垕食堂团购客户']);
            if(!$customNumIf){
                $this->model->rollback();
            ...
            if($key>=1){
                ...
                $orderSql.=" INTO sh_order (tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
                $orderSql.="{$val['cardno']},'{$val['num']}','01','{$map['panterid']}')";

                $teamSql.=" INTO sh_team(teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                ...
                //充值单记录
                $orderSql.="INSERT ALL INTO sh_order (tradeid,cardno,cardnum,flag,panterid) VALUES ('{$purchaseid}',";
                $orderSql.="{$val['cardno']},'{$val['num']}','01','{$map['panterid']}')";
                // 团冲记录写入
                $teamSql.="INSERT ALL INTO sh_team (teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                $teamSql.="'{$map['teamid']}','{$purchaseid}','{$placddate}','{$placdtime}','1')";
            }
            // 修改账户金额
    ...
    private function getTeamid($map){
        $map['flag']='01';
        $res=$this->model->table('sh_order')->where($map)->field('tradeid')->find();
        if($res==true){
            $teamlist=$this->model->table('sh_team')->where(['purchaseid'=>trim($res['tradeid']),'flag'=>'1'])->field('teamid')->find();
            if($teamlist==true){
    ...
    private function teamConsume($customlist){
        ...
        foreach($customlist as $val){
            ...
            $xiao=$this->model->table('sh_order')->where($xiaoWhere)->field('price,num')->select();
            if($xiao){
                foreach($xiao as $v){
            ...
            $tuicai=$this->model->table('sh_order')->where($tuiWhere)->field('price,num')->select();
            if($tuicai==true){
    ...
    */
    private function teamCombine($customlist){
        ...
        foreach($customlist as $val){
            ...
            $inlist=$this->model->table('sh_combine')->where($inWhere)->field('amount')->select();
            $in=0;
            if($inlist==true){
            ...
            $outlist=$this->model->table('sh_combine')->where($outWhere)->field('amount')->select();
            $out=0;
            if($outlist==true){
    ...
    public function returnTeam(){
        ...
        $cardInfo   = $this->cardInfo(__FUNCTION__,$cardno);
        $customInfo = $this->customInfo(__FUNCTION__,$cardInfo['customid']);
        $customInfo['customlevel']=='神垕食堂团购客户'||returnMsg(array('status'=>'07','codemsg'=>'此卡号不是团购卡:'.$cardno.'不能退卡'));
        $mapTeam = ['cardno'=>$cardno,'cardnum'=>$customInfo['num']];
        $teamid = $this->getTeamid($mapTeam);
    ...
    private function cardTeamInfo($method,$teamid){
        $lists = M('sh_team')->where(['teamid'=>$teamid])->select();
        $count = count($lists);
    ...
    private function exTeamCards($info,$map,$cardno){
        ...
        foreach($info as $key => $val){
            ...
            if($key>=1){
                ...
                $orderSql.=" INTO sh_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
                ...
                $sql.="'{$termno}','{$termno}','{$panterid}','{$tradeid}','{$placeddate}','{$val['amount']}','0','{$val['customid']}','{$val['cardno']}','{$placedtime}','{$tradetype}','{$tac}','0')";
                // sh_order 表
                $orderSql.="INSERT ALL INTO sh_order(tradeid,cardno,cardnum,flag,panterid) VALUES ('{$tradeid}',";
                $orderSql.="'{$val['cardno']}','{$val['num']}','03','{$panterid}')";
                //sh_team 表
                $teamSql.="INSERT ALL INTO sh_team (teamid,purchaseid,placeddate,placedtime,flag) VALUES (";
                $teamSql.="'{$teamid}','{$tradeid}','{$placeddate}','{$placedtime}','2')";
...
//------------------------------------------------------菜品处理----------------------------
    ...
    private function addGoods($map){
        $goodslist=M('sh_goods')->where(['panterid'=>$map['panterid']])->select();
        if($goodslist==false){
            $goodsid=1;
        }else{
            $namelist=M('sh_goods')->where(['goodsname'=>$map['goodsname'],'panterid'=>$map['panterid'],'type'=>$map['type']])->find();
            if($namelist==true){
        ...
        $sql="INSERT INTO sh_goods (goodsid,panterid,type,goodsname,price,status) VALUES ('{$goodsid}','{$map['panterid']}','{$map['type']}','{$map['goodsname']}','{$map['price']}','1')";
        return $this->model->execute($sql);
    }
    //新增菜品时  添加菜品对应的列别
    private function addType($map,$flag){
        $typelist=M('sh_type')->where(['panterid'=>$map['panterid'],'type'=>$flag])->find();
        if($typelist==false){
            $sql="INSERT INTO sh_type (panterid,type,name,status) VALUES ('{$map['panterid']}','{$flag}','{$map['typename']}','1')";
            return $this->model->execute($sql);
        }else{
            return true;
        }
    ...
    private function goodsInfo($map){
        if($gooodsInfo = M('sh_goods')->where($map)->find()){
            return $gooodsInfo;
        }else{
    ...
    private function updateGoodsStatus($where,$map){
        return M('sh_goods')->where($where)->save($map);
    }
 ...
 //------------------------------大食堂其他方法
    ...
    public function weekData(){
        ...
        if($panterid==C('shenhou_getDrink')){
            ...
            $list=M('sh_order')->where($map)->field('nvl(sum(price*num),0) esum,placeddate,flag')
                ->group('placeddate,flag')->order('placeddate desc')->select();
            //退菜的
            $map['flag']='04';
            $refundlist=M('sh_order')->where($map)->field('nvl(sum(price*num),0) esum,placeddate,flag')
                ->group('placeddate,flag')->order('placeddate desc')->select();
            //第一步 有退菜日期的 获取  与正常合并
    ...
    private function getDrink($panterid){
        ...
        $map['panterid']=$panterid;
        $map['flag']='02';
        $getlist=M('sh_order')->where($map)->field('sum(price*num) as csum,sum(num) as tnum,goodsname,goodsid,type,placeddate')
            ->group('goodsname,goodsid,type,placeddate')->select();
        //退菜
        $map['flag']='04';
        $refulist=M('sh_order')->where($map)->field('sum(price*num) as csum,sum(num) as tnum,goodsname,goodsid,type,placeddate')
            ->group('goodsname,goodsid,type,placeddate')->select();
        if($refulist==true){
...
//---------------------------------------------公用方法
    ...
    private function cardInfo($method,$cardno){
        if($cardInfo = $this->model->table('cards')->where(['cardno'=>$cardno])->find()){
            ...
            if(substr($cardno,3,4)!='0574'){
                $error= ['status'=>'02','codemsg'=>'此卡号:'.$cardno.'不属于神垕地区'];
                $this->errorHandle($method,$error,'查询卡号信息操作');
            }
            return $cardInfo;
        }else{
    ...
    private function valiPanterid($method,$panterid,$description){
        ...
        if($list['parent']!=$this->parent){
            $error=['status'=>'13','codemsg'=>'不是神垕大食堂商户'];
            $this->errorHandle($method,$error,$description);
        }
        return $list;
    }
    ...
    private function errorLog($method,$error,$description){
        $filename = PUBLIC_PATH.'logs/sh_catering_error/'.date('Ymd',time()).'.txt';
    ...
    private function writeLog($dir,$data){
        ...
        $str.="\n";
        $filename=PUBLIC_PATH.'logs/sh_catering/'.$dir.'/'.date('Ymd',time()).'.txt';
        $this->getDir($filename);
        file_put_contents($filename,$str,FILE_APPEND);
    }
    //合计金额
...
//		}
    ...
    public function giftRate()
    {
        ...
        returnMsg(['status'=>'1','gift'=>C('sh_gift_api')]);
    }
    ...
    protected function exGiftChong( $map){
        ...
        $orderSql="INSERT INTO sh_order(tradeid,cardno,cardnum,flag,panterid,price,placeddate,placedtime) VALUES ('{$purchaseid}',";
        $orderSql.="{$cardno},'{$num}','05','{$panterid}','{$realmoney}','{$currentDate}','{$placedtime}')";
        $orderif= $this->model->execute($orderSql);
        if($customplIf==true && $auditlogsIf==true && $cardplIf==true && $orderif==true ){
            return ['msg'=>true,'purchaseid'=>$purchaseid];
        }else{
