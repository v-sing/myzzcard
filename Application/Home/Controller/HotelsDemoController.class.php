...
class HotelsDemoController{
    ...
    function getTicketConsumeList(){
        ...
        $subQuery = "(select sum(tradeamount) as tradeamount,cardno,panterid,quanid,placeddate,placedtime,customid,flag ";
        $subQuery.="from trade_wastebooks where tradetype='02' and flag=0 group by cardno,panterid,placeddate,placedtime,quanid,customid,flag)";
...
//echo $model->getLastSql()
    ...
    function getBalanceList(){
      $this->setTimeMemory;
        ...
        if($cardno!=''){
            $cardno=explode(',',$cardno);
            $where['a.cardno']=array('in',$cardno);
        }
        if($customid!=''){
            $where['a.customid']=$customid;
        }
...
//                 $results[]=$v;
    ...
    function getProductQueryList(){
        ...
        $subQuery1.=" where d.customid=a.customid and a.quanid=c.quanid and  m.cid=d.customid and b.customid=m.customid and p.panterid=c.panterid and a.type='02' and a.quanid!='NULL' and d.cardno not in (select cardno from card_change_logs))";
        //充值数量
		...
		$sqlStr = "select sum(q.amount) as amount,q.customid,q.quanid,c.status
from quancz q 
inner join cards c on c.customid=q.customid  
where q.quanid in('00000012','00000018','00000044','00000050','00000066','00000086','00000092','20151120','20160201','20160101','00000021','00000037','00000035','00000010','00000060','00000073','00000082') and q.placeddate<20170424 and q.placeddate>20150201
group by q.customid,q.quanid,c.status";
              $res = $model->query($sqlStr); 
            echo count($res);exit;
        ...
        if($end!=''){
            ...
            foreach($results as $k=>$v){
                //补卡处理
                $whereHotel['flag']='1';
                $whereHotel['precardno']=$v['cardno'];
                //根据新卡查询相关旧卡信息
                $oldcards=M('card_change_logs')->where($whereHotel)
                ->order('placeddate desc')->find();
                if(count($oldcards)>0){
                    $quanid=M('cards')->alias('ca')
                        ->join('account ac on ca.customid=ac.customid')
                        ->where(array("ca.cardno"=>$oldcards['precardno']))->field("quanid")->select();
                    foreach($quanid as $m=>$n){
                        if(array_key_exists($oldcards['precardno'].$n['quanid'],$results)){
                            $wheres=$where;
                            $wheres['h.cardno']=$oldcards['cardno'];
                            $newCardlist=$model->table($subQuery1)->alias('h')
                            ->join('left join '.$subQuery2.' f  on h.customid1=f.customid and h.quanid=f.quanid')
                            ->join('left join '.$subQuery3.' i  on i.cardno=h.cardno and i.quanid=h.quanid')
                            ->join('left join '.$subchong.'  m  on h.customid1=m.customid and h.quanid=m.quanid')
                            ->join('left join '.$subtrade.'  n  on n.cardno=h.cardno')
                            ->where($wheres)->field($field1)->find();
                        $results[$v['precardno'].$n['quanid']]['recharamounts']=bcadd($results[$v['precardno'].$n['quanid']]['rechargeamount'],$newCardlist['rechargeamount']);//券最终充值数量
                        $results[$v['precardno'].$n['quanid']]['recharmoneys']=bcadd($results[$v['precardno'].$n['quanid']]['rechargeamount'],$newCardlist['rechargeamount'])*floatval($newCardlist['quanprice']);//券最终充值金额
                        $results[$v['precardno'].$n['quanid']]['tradeamounts'] =bcadd($results[$v['precardno'].$n['quanid']]['consumeamount'],$newCardlist['consumeamount']);//券最终消费数量
                        $results[$v['precardno'].$n['quanid']]['trademoneys'] =bcadd($results[$v['precardno'].$n['quanid']]['consumeamount'],$newCardlist['consumeamount'])*floatval($newCardlist['quanprice']);//券最终消费金额
                        $results[$v['precardno'].$n['quanid']]['amounts'] =bcadd($results[$v['precardno'].$n['quanid']]['amount'],$newCardlist['amount']);//券最终剩余数量
                        $results[$v['precardno'].$n['quanid']]['suramounts'] =bcadd($results[$v['precardno'].$n['quanid']]['amount'],$newCardlist['amount'])*floatval($v['quanprice']);//券最终剩余金额
                        $results[$v['precardno'].$n['quanid']]['newcard']=$v['cardno'];//补卡卡号
                        $results[$v['precardno'].$n['quanid']]['searamounts'] = bcadd($results[$v['precardno'].$n['quanid']]['searchamount'],bcsub($newCardlist['rechargeamount1'],$newCardlist['consumeamount1']));//券最终查询数量
                        $results[$v['precardno'].$n['quanid']]['searmoneys'] = bcadd($results[$v['precardno'].$n['quanid']]['searchamount'],bcsub($newCardlist['rechargeamount1'],$newCardlist['consumeamount1']))*floatval($newCardlist['quanprice']);//券最终查询金额
                        }  
                    }
                }    
                $v['quanprice']=floatval($v['quanprice']);//劵单价
                $v['recharamount']=floatval($v['rechargeamount']);//充值数量
                $v['recharmoney']=$v['quanprice']*floatval($v['rechargeamount']);//充值金额
                $v['tradeamount']=floatval($v['consumeamount']);//消费数量
                $v['trademoney']=$v['quanprice']*floatval($v['consumeamount']);//消费金额
                $v['searamount'] = bcsub($v['rechargeamount1'],$v['consumeamount1']);//查询数量=充值数量—交易数量
                $v['searmoney']= bcsub($v['rechargeamount1'],$v['consumeamount1'])*floatval($v['quanprice']);//查询金额
                $v['amount']=floatval($v['amount']);//剩余数量
                $v['suramount']=$v['quanprice']*floatval($v['amount']);//剩余金额
                unset($results[$k]);//销毁变量
                $results[$v['cardno'].$v['quanid']] =$v;
                //退卡问题
                $boo2 = M('return_cards_log')->where(array('cardno'=>$v['cardno'],flag=>'2'))->find();
                if(true==$boo2){
                    unset($results[$v['cardno']]);
                ...
                if(count($oldcards)>0){
                    ...
                    foreach($quanid as $m=>$n){
                        if(array_key_exists($oldcards['precardno'].$n['quanid'],$results)){
                            ...
                            $newCardlist =$model->table($subQuery1)->alias('h')
                            ->join('left join '.$subQuery2.' f  on h.customid1=f.customid and h.quanid=f.quanid')
                            ->join('left join '.$subQuery3.' i  on i.cardno=h.cardno')
                            ->where($wheres)->field($field)->find();
                            $results[$oldcards['precardno'].$n['quanid']]['recharamounts']=bcadd($results[$oldcards['precardno'].$n['quanid']]['rechargeamount'],$newCardlist['rechargeamount']);//券最终充值数量
                            $results[$oldcards['precardno'].$n['quanid']]['recharmoneys']=bcadd($results[$oldcards['precardno'].$n['quanid']]['rechargeamount'],$newCardlist['rechargeamount'])*floatval($newCardlist['quanprice']);//券最终充值金额
                            $results[$oldcards['precardno'].$n['quanid']]['tradeamounts'] =bcadd($results[$oldcards['precardno'].$n['quanid']]['consumeamount'],$newCardlist['consumeamount']);//券最终消费数量
                            $results[$oldcards['precardno'].$n['quanid']]['trademoneys'] =bcadd($results[$oldcards['precardno'].$n['quanid']]['consumeamount'],$newCardlist['consumeamount'])*floatval($newCardlist['quanprice']);//券最终消费金额
                            $results[$oldcards['precardno'].$n['quanid']]['amounts'] =bcadd($results[$oldcards['precardno'].$n['quanid']]['amount'],$newCardlist['amount']);//券最终剩余数量
                            $results[$oldcards['precardno'].$n['quanid']]['suramounts'] =bcadd($results[$oldcards['precardno'].$n['quanid']]['amount'],$newCardlist['amount'])*floatval($newCardlist['quanprice']);//券最终剩余金额
                            $results[$oldcards['precardno'].$n['quanid']]['searamounts'] ='';//查询数量
                            $results[$oldcards['precardno'].$n['quanid']]['searmoneys'] ='';//查询金额
                            $results[$oldcards['precardno'].$n['quanid']]['newcard']=$oldcards['cardno'];
                        }
                    }
                }
                $v['quanprice']=floatval($v['quanprice']);//劵单价
                $v['recharamount']=floatval($v['rechargeamount']);//充值数量
                $v['recharmoney']=$v['quanprice']*$v['rechargeamount'];//充值金额
                $v['tradeamount']=floatval($v['consumeamount']);//消费数量
                $v['trademoney']=floatval($v['consumeamount']*$v['quanprice']);//消费金额
                $v['searamount']='';//查询数量
                $v['searmoney']='';//查询金额
                $v['amount']=floatval($v['amount']);//剩余数量
                $v['suramount']=floatval($v['amount']*$v['quanprice']);//剩余金额
                unset($results[$k]);//销毁变量
                $results[$v['cardno'].$v['quanid']] =$v;
                //退卡问题
                $boo2 = M('return_cards_log')->where(array('cardno'=>$v['cardno'],flag=>'2'))->find();
        ...
        foreach($results as $m=>$n){
            if($n['newcard']){
                $n['fnrecharamount']=$n['recharamounts'];//充值数量
                $n['fnrecharmoney']=$n['recharmoneys'];//充值金额
                $n['fntradeamount']=$n['tradeamounts'];//消费数量
                $n['fntrademoney']=$n['trademoneys'];//消费金额
                $n['fnsearamount'] =$n['searamounts'];//查询数量=充值数量—交易数量
                $n['fnsearmoney']=$n['searmoneys'];//查询金额
                $n['fnamount']=$n['amounts'];//剩余数量
                $n['fnsuramount']=$n['suramounts'];//剩余金额
            }else{
                $n['fnrecharamount']=$n['recharamount'];//充值数量
                $n['fnrecharmoney']=$n['recharmoney'];//充值金额
                $n['fntradeamount']=$n['tradeamount'];//消费数量
                $n['fntrademoney']=$n['trademoney'];//消费金额
                $n['fnsearamount'] =$n['searamount'];//查询数量=充值数量—交易数量
                $n['fnsearmoney']=$n['searmoney'];//查询金额
                $n['fnamount']=$n['amount'];//剩余数量
                $n['fnsuramount']=$n['suramount'];//剩余金额
            }
            $rows[]=$n;
        }
        $result=array();
  ...
  public function yuanreturnCoupon(){
    ...
    public function giftConsumeVoucher(){
        ...
        foreach($data as $k=>$v){
            ...
            $tradeIf=$model->execute($tradeSql);
            if($result==true&&$cardresult==true&&$tradeIf==true){
                $model->commit();
                $str=1;
            }else{
                $str=2;
