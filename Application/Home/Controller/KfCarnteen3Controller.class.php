...
use Think\Model;
class KfCarnteen3Controller extends  CommonController
{
    protected $model;
    protected $parent;
    protected $panterid = '00000546';
    protected $pantername='开封九道弯';
    protected $frozen = 5;
    ...
    public function VerifBalance(){
        if(IS_POST){
            ...
            //session('cardnos',null);session('num',null);session('cardnoInfo',null);$this->ajaxReturn(array('status'=>0,'msg'=>'该卡已刷过卡,请更换卡'));
            if(session('cardnos')){
                $arr=session('cardnos');
            ...
            //$this->ajaxReturn(array('status'=>0,'msg'=>session('cardnos')));
            ...
            //$this->ajaxReturn(array('status'=>0,'msg'=>$cardnoInfo['num']));
                ...
                if((session('num')==$cardnoInfo['num'])||(session('num')!=0&&$cardnoInfo['num']!=0)){
                    if(session('num')!=0){
                        ...
                        if(bcsub($amount,$this->frozen,2)<=$total){//叠卡总额不足
                            ...
                            $this->ajaxReturn(array('status'=>0,'msg'=>'叠卡总额不足,还差'.($total-$amount-($this->frozen))));
                        }else{//叠卡总额充足
    ...
    private function setHmtl($val,$style=''){
        if($val['status']=='1'){
            return "<li {$style}>
                    ...
                    <div class='dishes_set'><a href='http://test.9617777.com/zzkp.php/KfCarnteen/editDish?goodsid={$val["goodsid"]}'><span style='color:#999;' >编辑</span></a><span style='color:#f88b0f;  margin-left:110px; display: inline-block;' value='{$val['goodsid']}' class='under'>下架</span></div>
            ...
            return "<li {$style}>
                    ...
                    <div class='dishes_set'><a href='http://test.9617777.com/zzkp.php/KfCarnteen/editDish?goodsid={$val["goodsid"]}'><span style='color:#999;' >编辑</span></a><span style='color:green;  margin-left:110px; display: inline-block;' value='{$val['goodsid']}' class='ondish'>上架</span></div>
    ...
    public function editDish(){
        if($_POST){
            ...
            $result=M('kf_goods')->where(array('goodsid'=>$goodsid))->save(array('goodsname'=>$goodsname,'price'=>$price,'type'=>$type));
            if($result){
            ...
            $result=M('kf_goods')->where(array('goodsid'=>$goodsid))->find();
    ...
    */
    ...
    public function daliyBalance(){
        ...
        $data=array(array('datetime'=>date('Y.m.d'),'consume'=>floatval($consume),'refund'=>floatval($refund)));
        return $data;
    }
