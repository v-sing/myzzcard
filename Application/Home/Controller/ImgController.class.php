...
class ImgController extends CoinController{
   public function getImg(){
    ...
    //  $customid='MDAwMDM2MTEO0O0O';
		//  $customid = encode('10000013');
     $customid = decode($customid);
		 $customid1 = encode($customid);
     $customlist = $this->customs->where(array('customid'=>$customid))->find();
     $customlist==true || returnMsg(array('status'=>'05','codemsg'=>'会员编号不存在'));
		 ...
		 $url = C('ApiUrl');
		$content= $url."/zzkp.php/Img/getImg?customid={$customid1}&linktel={$customlist['linktel']}&rand=".time();
		//  include 'phpqrcode.php';
		// require PUBLIC_PATH."phpqrcode.php";
		// $QRcode = new QRcode();
 		$errorCorrectionLevel = 'L';//容错级别
 		$matrixPointSize = 10;//生成图片大小
		// 	$openid=$user1['openid'];
	    $customid = $customid.rand(0,99999999);
      ...
      $filename = ltrim($filename,'.');
 			$erweima=$filename;
 			returnMsg(array('status'=>'1','img'=>$erweima));exit;
 		}else{
 			QRcode::png($content, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
 		// 	$_SESSION['qrcode_time']=time();
     $filename = ltrim($filename,'.');
