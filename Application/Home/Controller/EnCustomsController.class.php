...
class EnCustomsController extends CommonController {
    ...
    function index(){
        ...
        if($panterid!='FFFFFFFF'){
            ...
            $where['_string']="(p.panterid not in ('00000125','00000127','00000118','00000270','00000126','00000509','00000241') or p.panterid is null )and c.namechinese IS NOT NULL ";
            $is_admin=1;
        }
        switch ($orderby) {
            case 'customid':
...
//                }
            ...
            $count=$model->table('encustoms')->alias('c')
            ...
            $custom_list=$model->table('encustoms')->alias('c')
                ->join('left join __PANTERS__ p on c.panterid=p.panterid')
                ->join('left join __CITY__ ct on ct.cityid=c.cityid')
                ->join('left join __COUNTY__ co on co.countyid=c.countyid')
                ->join('left join '.$subQuery.' b on b.customid=c.customid')
        ...
        $page = $p->show();
        $custom_list = $this->customDecode($custom_list);
        if($is_admin==1){
...
//        }
    ...
    public function add(){
      ...
      // $hysxlist = M('panters')->where(array('panterid'=>$panterid_if))->field('hysx')->find();
        if(IS_POST){
            ...
            $linktel = $numEncode->encode($linktel,'1');
            $personid = $numEncode->encode($personid);
            $sql='insert into encustoms(customid,namechinese,nameenglish,cityid,linktel,';
            ...
            $sql1='insert into customs(customid,namechinese,nameenglish,cityid,linktel,';
            $sql1.='birthday,customlevel,sex,personidtype,personid,email,staffpaper,';
            $sql1.="residaddress,unitzip,countrycode,linkman,panterid,placeddate,frontimg,reserveimg,personidexdate,countyid,career ) values('{$customid}','{$namechinese}',";
            $sql1.="'{$nameenglish}','{$cityid}','{$linktel}','{$birthday}','{$customlevel}',";
            $sql1.="'{$sex}','{$personidtype}','{$personid}','{$email}','{$staffpaper}'";
            $sql1.=",'{$residaddress}','{$unitzip}','{$countrycode}','{$linkman}','{$panterid}','{$currentDate}','{$frontImg}','{$reserveImg}','{$personidexdate}','{$countyid}','{$career}')";
            if($customs->execute($sql)){
                $customs->execute($sql1);
                $customs->commit();
    ...
    public function edit(){
      ...
      //后台验证身份证和电话唯一性
            ...
            $custom_info=M('encustoms')->alias('cu')->join("left join panters p on p.panterid=cu.panterid")->field($field)->where($where)->find();
            $custom_info = $this->customDecode($custom_info);
    ...
    protected function customDecode($arr){
        $numEncode = new NumEncode();
        $oneLevel = $this->arrOneLevel($arr);
        if($oneLevel){
            $arr['linktel'] = $numEncode->decode( $arr['linktel'],'1');
            $arr['personid'] =$numEncode->decode( $arr['personid']);
        }else{
            foreach($arr as $key=>$val){
                if($val['linktel']){
                    $val['linktel'] = $numEncode->decode( $val['linktel'],'1');
                }
                if($val['personid']){
                    $val['personid'] =$numEncode->decode( $val['personid']);
                }
                $arr[$key] = $val;
            }
        }
    ...
    protected function customEncode($arr){
        $numEncode = new NumEncode();
        $oneLevel = $this->arrOneLevel($arr);
        if($oneLevel){
            $arr['linktel'] = $numEncode->encode( $arr['linktel'],'1');
            $arr['personid'] =$numEncode->encode( $arr['personid']);
        }else{
            foreach($arr as $key=>$val){
                if($val['linktel']){
                    $val['linktel'] = $numEncode->encode( $val['linktel'],'1');
                }
                if($val['personid']){
                    $val['personid'] =$numEncode->encode( $val['personid']);
                }
                $arr[$key] = $val;
            }
        }
