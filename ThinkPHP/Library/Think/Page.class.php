...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'末页','theme'=>'%totalRow% %header% %nowPage%/%totalPage% 页%upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%','themesd'=>' %upPage% %downPage% %first%  %prePage%  %linkPage% <li><a href="#">%totalRow% %header%</a></li> <li><a href="#">%nowPage%/%totalPage% 页</a></li> %nextPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show2() {
        ...
        $pageStr     =   str_replace(
            array('%header%','%nowPage%','%totalRow%','%totalPage%','%upPage%','%downPage%','%first%','%prePage%','%linkPage%','%nextPage%','%end%'),
            array($this->config['header'],$this->nowPage,$this->totalRows,$this->totalPages,$upPage,$downPage,$theFirst,$prePage,$linkPage,$nextPage,$theEnd),$this->config['themesd']);
        return $pageStr;
    }
