...
class Model {
    ...
    protected function _before_update(&$data,$options) {
        $ip=$this->getIP();
        $str='ip:'.$ip.'||'.date('Y-m-d  H:i:s',time()).'：'.$_SESSION['username'].'人员--save操作之前数据：表名字-- '.$this->name.'||更新数据>>>';
        foreach($data as $key => $val){
          $str.=$key.'=>'.$val.",";
        }
        $str.='||条件是：';
        ...
        $str.=$this->getLastSql()."\n";
        $date=date('Ymd',time());
        $filename=PUBLIC_PATH.'sys/before_save/'.$date.'.txt';
    ...
    protected function _after_update($data,$options) {
        $ip=$this->getIP();
        $str='ip:'.$ip.'||'.date('Y-m-d  H:i:s',time()).'：'.$_SESSION['username'].'人员--save操作： ';
        $date=date('Ymd',time());
        $str.=$this->getLastSql()."\n";
        $filename=PUBLIC_PATH.'sys/save/'.$date.'.txt';
        $path = pathinfo($filename);
        $path = $path['dirname']."/";
        file_exists($path) || mkdir($path,0777,true);
        file_put_contents($filename,$str,FILE_APPEND);
    ...
    protected function _before_delete($options) {
        $ip=$this->getIP();
        $str='ip:'.$ip.'||'.date('Y-m-d  H:i:s',time()).'：'.$_SESSION['username'].'人员--delete操作before：》》》表名： '.$options['table'].'(';
        $date=date('Ymd',time());
        foreach($options['where'] as $key => $val){
           $str.=$key.'=>'.$val;
        }
        $str.=")\n";
        $filename=PUBLIC_PATH.'sys/before_delete/'.$date.'.txt';
        $path = pathinfo($filename);
        $path = $path['dirname']."/";
        file_exists($path) || mkdir($path,0777,true);
        file_put_contents($filename,$str,FILE_APPEND);
    ...
    protected function _after_delete($data,$options) {
        $ip=$this->getIP();
        $str='ip:'.$ip.'||'.date('Y-m-d  H:i:s',time()).'：'.$_SESSION['username'].'人员--delete操作： ';
        $date=date('Ymd',time());
        $str.=$this->getLastSql()."\n";
        $filename=PUBLIC_PATH.'sys/delete/'.$date.'.txt';
        $path = pathinfo($filename);
        $path = $path['dirname']."/";
        file_exists($path) || mkdir($path,0777,true);
        file_put_contents($filename,$str,FILE_APPEND);
    ...
    protected function _after_select(&$resultSet,$options) {
        $ip=$this->getIP();
        $str='ip:'.$ip.'||'.date('Y-m-d  H:i:s',time()).'：'.$_SESSION['username'].'人员--查询操作： ';
        $str.=$this->getLastSql()."\n";
        $date=date('Ymd',time());
        $filename=PUBLIC_PATH.'sys/select/'.$date.'.txt';
        $path = pathinfo($filename);
        $path = $path['dirname']."/";
        file_exists($path) || mkdir($path,0777,true);
        file_put_contents($filename,$str,FILE_APPEND);
    ...
    public function execute($sql,$parse=false) {
        ...
        $sql  =   $this->parseSql($sql,$parse);
        if(false===$this->_after_execute($sql)){
            return false;
        }
        return $this->db->execute($sql);
    }
    protected  function _after_execute($sql){
        $ip=$this->getIP();
        $str='ip:'.$ip.'||'.date('Y-m-d  H:i:s',time()).'：'.$_SESSION['username'].'人员--execute操作： ';
        $str.=$sql."\n";
        $date=date('Ymd',time());
        $filename=PUBLIC_PATH.'sys/execute/'.$date.'.txt';
        $path = pathinfo($filename);
        $path = $path['dirname']."/";
        file_exists($path) || mkdir($path,0777,true);
        file_put_contents($filename,$str,FILE_APPEND);
