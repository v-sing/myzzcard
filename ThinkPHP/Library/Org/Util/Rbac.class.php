...
class Rbac {
    ...
    static public function getAccessList($authId) {
        ...
        $sql    =   "select node1.id,node1.name from ".
                    $table['role']." role1,".
                    $table['user']." user1,".
                    $table['access']." access1,".
                    $table['node']." node1 ".
                    "where user1.user_id='{$authId}' and user1.role_id=role1.id and ( access1.role_id=role1.id  or (access1.role_id=role1.pid and role1.pid!=0 ) ) and role1.status=1 and access1.node_id=node1.id and node1.lev=1 and node1.status=1";	
        $apps =   $db->query($sql);
        $access =  array();
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node1.id,node1.name from ".
                    ...
                    $table['access']." access1 ,".
                    $table['node']." node1 ".
                    "where user1.user_id='{$authId}' and user1.role_id=role1.id and ( access1.role_id=role1.id  or (access1.role_id=role1.pid and role1.pid!=0 ) ) and role1.status=1 and access1.node_id=node1.id and node1.lev=2 and node1.pid={$appId} and node1.status=1";
            ...
            foreach($modules as $key=>$module) {
                ...
                $sql    =   "select node1.id,node1.name from ".
                    ...
                    $table['access']." access1 ,".
                    $table['node']." node1 ".
                    "where user1.user_id='{$authId}' and user1.role_id=role1.id and ( access1.role_id=role1.id  or (access1.role_id=role1.pid and role1.pid!=0 ) ) and role1.status=1 and access1.node_id=node1.id and node1.lev=3 and node1.pid={$moduleId} and node1.status=1";
                $rs =   $db->query($sql);
                $action = array();
                foreach ($rs as $a){
                    $action[$a['name']]	 =	 $a['id'];
